...
## D-030: cross-repo flow — `git pull` + правила целевого репо обязательны (12.05.2026)
**Контекст:** TASK-011 (IATA-732 dashboard). Агент Obligations напрямую правил файлы в `/home/budnik_an/todo/` (`backend/app.py`, `webBI/info.html`), не сделав `git pull` и не прочитав `.cursor/rules/` целевого репозитория. Локальная рабочая копия `todo/` была устаревшей на десятки коммитов (Keycloak SSO, новый стандарт навигации `portal-navigation.mdc`, on-demand alerts, publisher-сабагент для HTML). В результате `git checkout HEAD -- backend/app.py webBI/info.html` второго агента пришлось делать руками, а pristine версия из docker-образа, которой я попытался "починить" контейнер, тоже была устаревшей и снесла рабочие фичи. Регресс пришлось ликвидировать целевому агенту коммитом `cb31846`. См. [TASK-011 plan](../.cursor/plans/TASK-011_iata732_dashboard.md) § «Lessons».

**Решение:**
- При любой работе в чужом репозитории (D-023 не отменяется, расширяется):
  1. **Сначала `git pull origin <branch>`** — синхронизация с remote, проверка что HEAD совпадает с origin.
  2. **Прочитать `.cursor/rules/*.mdc` целевого репо** — в каждом репозитории могут быть собственные ограничения (запрет на прямую правку HTML, наличие publisher-сабагентов, code owners).
  3. **Проверить divergence** с docker-образом: `git log --oneline origin/main..HEAD` + сверить с image build args. Production-образ может быть собран из новейшего коммита, расходящегося с локальной копией.
  4. **Если правила запрещают прямую правку** (HTML, frontend, миграции БД) — делегировать через подходящего сабагента в целевом репозитории, не через прямой StrReplace.
- Запрещено в кросс-репо сценариях:
  - Любой PUT в production-контейнер без предварительного `git pull` локальной копии исходника и сравнения с image.
  - Использование "pristine" версии из image как источника истины — она по определению старее текущей разработки.
  - Прямая правка HTML/CSS/JS в `todo/webBI/`, если в `todo/.cursor/rules/` объявлен publisher-сабагент.

**Альтернативы отклонены:**
- "Доверять локальной копии без `git pull`" — приводит к регрессу, доказано TASK-011.
- "Не работать в чужих репозиториях вообще" — нарушает D-023, делает интегральные задачи невозможными.

**Последствия:**
- Время каждой кросс-репо задачи увеличивается на 1–2 мин на `git pull` и чтение чужих правил — приемлемо.
- Производственные образы остаются стабильными — нет случайных регрессов от устаревших pristine.
- **Вернуться если:** появится автоматизация (CI-hook), которая проверяет state локальной копии vs remote перед PUT в контейнер.

## D-032: Override-таблица для сдвига колонки IATA в Excel-кодификаторе (2026-05-14)
**Контекст:** В листе `ЦУП_МЕР_IATA` строки r72–r80 содержат сдвиг колонки D (IATA-код)
на одну позицию вниз относительно колонки C (MER-код). Причина — два внутренних
кода ЮТэйр (82.1 «Ковёр», 83.1 «военные ограничения») вставлены без выравнивания.

**Решение:** применять `MER_IATA_OVERRIDE` в `build_mer_to_732_mapping.py`, восстанавливать
правильные пары по смысловому совпадению MER-описания и IATA-730 name_en. Источник
не правится (Rule 0 — фиксируется как факт о документе). В UI override-коды
помечаются 🔧 + подсказкой.

**Альтернативы отклонены:**
- Не делать override и оставить все r72–r80 в kind=gap. Отвергнута —
  80% этих строк имеют однозначное смысловое совпадение; не показать его было бы
  скрытием существующего соответствия.

## D-033: Дельта-CSV в формате SAP ОЗМ-style для апгрейда Меридиана (2026-05-14)
**Контекст:** Запрос human на CSV-выгрузку, готовую для импорта в справочник
Меридиана. Не сырой дамп IATA-732, а список изменений (что добавить, что
перепривязать, что пометить deprecated).

**Решение:** Формат «одна строка = одна атомарная операция», колонка `action` ∈ {ADD,
RENAME, REMAP, DEPRECATE, NO-CHANGE}. 16 колонок (mer_code_target, mer_name_current,
mer_name_proposed, mer_group, iata730_code, iata732_target, iata732_group, …).
Файл `output/cup_codifier/meridian_upgrade_v1.csv`, доступен через кнопку в дашборде
`/info/iata732/`. Шапка с `#`-комментариями; параметр `--no-header-comments` для
строгих парсеров не реализован, но колонки стандартные и парсятся напрямую.

**Альтернативы отклонены:**
- XLSX по специальному шаблону Меридиана. Отвергнута — human сказал «CSV пойдёт»,
  шаблона Меридиана нет на руках.

## D-031: Корректное написание бренда — «ЮТэйр» (13.05.2026)
**Контекст:** В кодовой базе и отчётах встречаются оба варианта — «ЮТэйр» (правильный, официальный) и «Утайр» (транслит, неправильно). Пользователь явно зафиксировал корректное написание.

**Решение:**
- Во всех **новых** текстах (отчёты в `docs/reports/`, планы в `.cursor/plans/`, commit messages, frontend-копирайт в `todo/webBI/`, code comments) использовать **«ЮТэйр»**.
- Существующие тексты массово не переписывать (`Утайр` остаётся в исторических артефактах) — заменять только при правке в этих же местах по другим причинам.
- В английских текстах остаётся `Utair` (это латинское написание бренда, корректное).

**Альтернативы отклонены:**
- Запустить shotgun-edit по всему репо — нарушит Rule 0 (массовая переразметка без явной задачи), создаст big diff без бизнес-смысла.
- Оставить как есть — продолжит разнобой.

**Последствия:**
- При правке любого текста рядом с упоминанием бренда — заменить `Утайр → ЮТэйр` точечно.
- При генерации новых отчётов/комментариев агенты используют только `ЮТэйр`/`Utair`.

## D-034: ATFM-приоритет в маппинге AHM 730 → IATA-732 (2026-05-14)
**Контекст:** При наивном двухбуквенном lookup `code2_index` коды AHM 730 ATFM
(AT/AX/AE/AW/AM/AS) попадали в семантически неверные позиции IATA-732.
Пример: `AW` матчился на `G1/A/W` = wildlife в arrival, тогда как правильная
цель — `G7/Z/W · weather × ATFM destination`.

**Решение:** В `build_matching_json.py` добавлен явный словарь `IATA730_ATFM_TO_732`,
имеющий **приоритет** над `code2_index`. Для шести ATFM-кодов (AT, AX, AE, AW, AM, AS)
маппинг идёт через `IATA730_ATFM_TO_732` по семантике AHM 730 → IATA-732 G7/Z/*
со stakeholders O/P/Q/S. `code2_index` применяется только как fallback для остальных
кодов.

**Альтернативы отклонены:**
- Использовать `code2_index` и пометить ATFM-коды как low-confidence. Отвергнута —
  семантический mismatch слишком велик (ATFM en-route vs wildlife в G1/A); это скрыло бы
  суть AHM 730 для разработчика Меридиана.

**Affected:**
- `output/cup_codifier/build_matching_json.py`: добавлен `IATA730_ATFM_TO_732`,
  изменён порядок проверок в `enrich_mer_entries`.
- Output: `mer_codes_atfm_mapped = 6`, DEPRECATE в CSV сократилось с 9 до 6 строк
  (итог: ADD=3, REMAP=484, DEPRECATE=6).

## D-035: Уточнение семантики внутренних кодов ЮТэйр 82.1 и 83.1 (2026-05-14)
**Контекст:** D-032 записал 82.1 как «Ковёр» и 83.1 как «военные ограничения» —
рабочие ярлыки из первичного анализа. Human уточнил фактическое содержание кодов
2026-05-14, что потребовало пересмотра IATA-732-таргетов.

**Решение:**
- **82.1 «режим Ковёр»** — угроза дронов + военные риски, закрытие воздушного
  пространства. IATA-732 main target: `G7/Z/P · military activity`, stakeholder T
  (local ATC destination). Alternative: `G7/Z/R · additional security event`,
  stakeholder S (security service).
- **83.1** — НЕ «военные ограничения», а **правительственные ограничения** (полёт
  VIP-борта). IATA-732 target: `G7/Z/I · special flights/VIP`, stakeholder O
  (ATFM departure regulation).

Уточнение закреплено в `INTERNAL_UTAIR_TARGETS` в `build_matching_json.py` и в
note-полях `MER_IATA_OVERRIDE` в `build_mer_to_732_mapping.py`.

**Альтернативы отклонены:**
- Оставить ярлык «военные» для 83.1. Отвергнута — семантически неверно; VIP-рейс
  регулируется отдельным ATFM-режимом, а не военной активностью.
- Объединить с D-032 как дополнение. Отвергнуто — D-032 фиксирует техническое правило
  (override Excel-сдвига), D-035 фиксирует фактическое содержание кода (источник:
  устное уточнение human); разные природы решений.

## D-036: Hooks enforcement отключены до миграции под Task-sub-agents (2026-05-14)
**Контекст:** `.cursor/hooks/hooks.json` подключал 4 хука enforcement
(`check_handoff.py`, `block_orchestrator_code.py`, `check_ownership.py`,
`record_stop.py`), реализующих жёсткое разграничение зон агентов из
`00_global_always.mdc`. Все три enforcement-хука идентифицируют агента через
`os.environ["CURSOR_AGENT_NAME"]`, которая выставляется только в модели «отдельный
Cursor-чат на агента» (D-005).

С переходом на модель «главный агент + sub-agents через Task tool» (D-011)
env-переменная **не пробрасывается** в child-процессы хуков → `agent="unknown"` →
все три хука пропускают всё (фактически бесполезны), либо при случайном
выставлении env начинают резать легитимные операции (например `git commit` от
имени human, если кто-то выставит `CURSOR_AGENT_NAME=orchestrator`).

**Решение:**
- Конфиг переименован: `.cursor/hooks/hooks.json` → `.cursor/hooks/hooks.json.disabled`.
- Сами скрипты в `.cursor/hooks/*.py` сохранены — пригодятся при переписывании
  под Task-sub-agents.
- Enforcement остаётся **soft** через правила в `00_global_always.mdc` (раздел
  Handoff-цепочка + ownership-таблица). Агенты следуют им декларативно, без
  hard-stop на уровне Cursor.

**Альтернативы отклонены:**
- Включить только `record_stop.py` (безопасный, лишь логирует). Отвергнуто —
  Cursor не поддерживает выборочное подключение хуков по одному, нужно
  переписывать `hooks.json` с тремя пустыми массивами, что путает картину;
  телеметрия завершения сессий не используется в текущем процессе.
- Переписать хуки под идентификацию агента из transcript / маркера в промпте.
  Отвергнуто как отдельная задача (тянет на TASK-013), для текущего цикла не
  блокирующее.
- Удалить хуки полностью. Отвергнуто — скрипты остаются полезным справочником
  по ownership-зонам и могут быть оживлены при возврате к D-005-модели.
