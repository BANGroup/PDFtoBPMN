...
### Проблема 5: "No module named 'pdf_to_context'" при запуске OCR сервиса

**Симптом:**
```
ERROR: ModuleNotFoundError: No module named 'pdf_to_context'
Application startup failed. Exiting.
```

**Причина:**
- Абсолютный импорт в `app.py`: `from pdf_to_context.ocr_service.prompts import OCRPrompts`
- Запуск через uvicorn из корня проекта → Python не находит модуль
...
**Проверка:**
```bash
# Не должно быть абсолютных импортов в ocr_service/
grep "from pdf_to_context" scripts/pdf_to_context/ocr_service/*.py

# Должно быть пусто (exit code 1 = OK)
```
...
### Проблема 6: Все изображения получают ошибку "cannot identify image file"

**Симптом:**
```
ERROR: OCR error 500 для ВСЕХ изображений
❌ Невалидное изображение: cannot identify image file <_io.BytesIO>
```

**Причина:**
- Жесткая проверка PIL в `app.py` блокирует изображения
- PIL не поддерживает PDF-специфичные форматы (CMYK, JPEG2000, векторная графика)
- DeepSeek-OCR имеет собственный internal parser и может обработать эти форматы

**✅ РЕШЕНИЕ:**

**НЕ использовать** проверку PIL перед отправкой в модель:

```python
# ❌ НЕПРАВИЛЬНО (костыль):
image = Image.open(io.BytesIO(image_data))
image.verify()  # Блокирует PDF форматы!
if not image:
    raise HTTPException(400, "Invalid image")

# ✅ ПРАВИЛЬНО (как в MVP):
# Сохранять RAW байты напрямую
with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp_file:
    tmp_file.write(image_data)  # DeepSeek-OCR сам разберется
    temp_path = tmp_file.name
```

**Проверка:**
- Обработка документа должна показать: `Обработано OCR: > 0`
- В логах НЕ должно быть: `cannot identify image file`

---

### Проблема 7: Сервис загрузился, но сразу Shutdown

**Симптом:**
```
INFO: ✅ DeepSeek-OCR успешно загружен!
INFO: Uvicorn running on http://0.0.0.0:8000
INFO: Shutting down
INFO: Application shutdown complete.
```

Обработка документа получает: `Connection refused`

**Причина:**
- Запуск через `nohup ... &` в том же вызове терминала с другими командами
- Процесс получает signal для завершения от родительского shell

**✅ РЕШЕНИЕ:**

**Вариант 1: Отдельный терминал**
```bash
# Терминал 1 (только для OCR сервиса)
cd $PROJECT_ROOT
source venv/bin/activate
python -m uvicorn scripts.pdf_to_context.ocr_service.app:app --host 0.0.0.0 --port 8000
```
