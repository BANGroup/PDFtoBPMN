...
class MDParser:
    ...
    def parse_md_content(self, content: str) -> List[OwnerRecord]:
        ...
        # Паттерн для кодов (может быть с | в таблицах или без)
        code_pattern = r'предоставляющим данные\s*\|?\s*(\d+_\d+)'
        ...
        for code_match in re.finditer(code_pattern, content):
            ...
            next_code_match = re.search(
                r'предоставляющим данные\s*\|?\s*\d+_\d+',
            ...
            # Создаем запись (даже если нет всех полей - заполним позже)
            record = OwnerRecord(
            ...
            # Добавляем запись (валидация не требуется - нужны все записи)
            records.append(record)
        
        # Дедупликация: каждый код встречается 2 раза (в таблице и в тексте)
        # Оставляем запись с максимальным количеством заполненных полей
        unique_records = {}
        for rec in records:
            if rec.owner_code not in unique_records:
                unique_records[rec.owner_code] = rec
            else:
                # Сравниваем заполненность полей
                existing = unique_records[rec.owner_code]
                existing_score = sum([
                    bool(existing.full_name),
                    bool(existing.address),
                    bool(existing.quantity),
                    bool(existing.document_number)
                ])
                new_score = sum([
                    bool(rec.full_name),
                    bool(rec.address),
                    bool(rec.quantity),
                    bool(rec.document_number)
                ])
                
                # Оставляем запись с лучшей заполненностью
